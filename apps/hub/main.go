...
func main() {
	...
	// 统一 URL 前缀 /ai-collab-hub（剥离后走路由 /api、/approval 等）
	r.Use(func(c *gin.Context) {
		if strings.HasPrefix(c.Request.URL.Path, "/ai-collab-hub/") {
			c.Request.URL.Path = strings.TrimPrefix(c.Request.URL.Path, "/ai-collab-hub")
		} else if c.Request.URL.Path == "/ai-collab-hub" {
			c.Request.URL.Path = "/"
		}
		c.Next()
	})
	...
	r.GET("/approval/:execID/approve", func(c *gin.Context) {
		...
		if err != nil {
			c.Data(200, "text/html; charset=utf-8", []byte(fmt.Sprintf("<h1>❌ 审批失败</h1><p>%s</p><p><a href='/ai-collab-hub'>返回</a></p>", err.Error())))
			return
		}
		c.Data(200, "text/html; charset=utf-8", []byte("<h1>✅ 已批准</h1><p>流程已恢复执行。</p><p><a href='/ai-collab-hub'>返回</a></p>"))
	})
	...
	r.GET("/approval/:execID/reject", func(c *gin.Context) {
		...
		if err != nil {
			c.Data(200, "text/html; charset=utf-8", []byte(fmt.Sprintf("<h1>❌ 操作失败</h1><p>%s</p><p><a href='/ai-collab-hub'>返回</a></p>", err.Error())))
			return
		}
		c.Data(200, "text/html; charset=utf-8", []byte("<h1>❌ 已拒绝</h1><p>流程已终止。</p><p><a href='/ai-collab-hub'>返回</a></p>"))
	})
	...
	// 共享资源下载 (无需鉴权)
	// ═══════════════════════════════════════════
	resourceRoot := "/root/aispace/ai-collab-hub/shared-resources"
	...
	authGroup.Use(auth.AuthMiddleware())
	{
		...
		authGroup.GET("/files", func(c *gin.Context) {
			...
			legacyPath := "/root/aispace/ai-collab-hub"
